...
var NoDefault = false;
var gColorObj, rainbowpickerStrings;
...
function changeHex()
{
   ...
   hex = hex.match(/^\#([0-9a-f]{6}|[0-9a-f]{3})$/i)
   if (!hex)
   {
     alert(rainbowpickerStrings.getString('rainbowpicker.colorNotHash'));
     return;
   }

   // XXX Convert three hex colors to six hex colors
   if (hex[1].length == 3)
   {
     hex[1] = hex[1].charAt(0) + hex[1].charAt(0) + hex[1].charAt(1) + hex[1].charAt(1) + hex[1].charAt(2) + hex[1].charAt(2);
   }

   colours.setHex(hex[1].toUpperCase());
   redrawEverything();
}
...
function StartUp()
{
  ...
  rainbowpickerStrings = document.getElementById('rainbowpickerStringBundle');
...
function ValidateData()
{
  ...
  if (!gColor)
  {
    ...
    alert(rainbowpickerStrings.getString('rainbowpicker.badInput'));
...
function onAccept()
{
  ...
  gColorObj.elemCurrColor = gColor.toUpperCase();
...
{
  var namedColour = elt.value.toLowerCase();
